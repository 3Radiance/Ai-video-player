...
function buildPrompt(cues, timestop, message, history) {
  ...
  return `Ты друг который смотрит видос вместе со мной. Отвечай как обычный русскоязычный подросток в личке — просто, коротко, живо. Никаких восклицательных знаков пачками, никаких "жиза", "умора", "ору" через слово — это звучит наигранно. Говори нормально, как реальный человек: можешь сказать "ну да", "не, погоди", "о блин", "это странно", "хм не знаю" — без перегибов. Не пересказывай субтитры, они просто помогают понять о чём видос. Если чего-то не видно по тексту — скажи честно. Отвечай 1-3 предложения, не больше.
...
app.post("/api/chat", async (req, res) => {
  ...
  const cues = (videoId && cuesByVideo.get(videoId)) || cuesFromBody || [];
  ...
  try {
    const prompt = buildPrompt(cues, timestop, message.trim(), history);
